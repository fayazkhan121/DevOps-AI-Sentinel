...
export function requireAuth(req: AuthedRequest, res: Response, next: NextFunction): void {
  ...
  try {
    ...
    const db = getDb();
    const session = db.get<{ is_active: number; expires_at: string }>(
    ...
    const user = db.get<{ is_active: number; role: string; permissions: string; org_id: string; username: string }>(
    ...
    const allowlist = db.all<{ cidr: string }>('SELECT cidr FROM ip_allowlist WHERE org_id = ?', [user.org_id]);
    ...
    db.run('UPDATE sessions SET last_activity = ? WHERE id = ?', [new Date().toISOString(), payload.sessionId]);
    next();
  } catch {
